...
const cardClass =
  "group flex flex-col overflow-hidden rounded-2xl bg-surface shadow-card ring-1 ring-line/15 transition duration-300 hover:-translate-y-0.5 hover:shadow-lift dark:shadow-none dark:ring-line/10 dark:hover:ring-accent/20";
  ...
  return (
    <article className={cardClass}>
      <div className={`relative overflow-hidden ${mediaClassName}`}>
        ...
        <span className="absolute right-4 top-4 flex size-8 items-center justify-center rounded-full bg-surface/90 font-quirk text-sm text-ink-muted shadow-soft ring-1 ring-line/15 dark:bg-surface/95 dark:ring-line/10">
          {badge}
        </span>
      </div>
      <div className="flex flex-1 flex-col gap-4 p-6 sm:p-8">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-ink-soft">{p.period}</p>
          <h2
            className={`mt-1 font-display font-semibold tracking-tight text-ink ${titleClassName ?? "text-xl"}`}
          >
            {p.name}
          </h2>
          {p.subtitle ? (
    ...
    <main className="mx-auto max-w-6xl px-5 pb-28 pt-14 sm:px-8 sm:pt-18">
      ...
      <div className="mt-14">
        <ProjectCard
          ...
          badge="#1"
          mediaClassName="aspect-[21/10] sm:aspect-[2/1]"
